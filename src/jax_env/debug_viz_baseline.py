#!/usr/bin/env python3
"""
debug_viz_baseline.py — Pygame Debug Visualizer (Baseline Environment)
=======================================================================
Usage:
  python debug_viz_baseline.py                    # random policy
  python debug_viz_baseline.py --checkpoint path  # trained policy
  python debug_viz_baseline.py --speed 0.5        # slower playback
  python debug_viz_baseline.py --pause            # start paused
"""
...
# Import baseline environment constants and functions
from jax_env import (reset_env, step_env, ROOM_W, ROOM_H, ROBOT_RADIUS, 
                     PEOPLE_RADIUS, DT, MAX_STEPS, GOAL_RADIUS, USE_LEGS, 
...
def draw_humans(surface, state, foot_state_np, show_arrows, use_legs, show_body):
    ...
    for i in range(n):
        # jax_env.py does not have a dummy mask (index 10)
...
def draw_panel(surface, fonts, ep, step, ep_ret, max_v, v, w,
    ...
    txt("─ BASELINE DEBUG ─", C_TEXT, "big"); y += 2; sep()
    
    txt(f"Scen  Baseline (0)", C_SUCCESS, "small")
...
def main():
    ...
    pygame.display.set_caption("RL Nav Debug (Baseline Viz)")
    ...
    while running:
        ...
        if (not paused or step_once) and not last_done:
            ...
            if ep_steps == 1:
                print(f"\n{'Step':>5} | {'Reward':>8}")
                print("─" * 18)
            print(f"{ep_steps:5d} | {last_r:+8.3f}")
