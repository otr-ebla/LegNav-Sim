...
def draw_scene(surface, state, raw_lidar, show_lidar, show_arrows):
    ...
    for i in range(int(state.people.shape[0])):
        ...
        theta  = float(state.people[i, 4]) # Extract global heading angle
        ...
        if show_arrows and speed > 0.05:
            # Draw a 0.5m long arrow strictly using the global heading angle (theta)
...
def main():
    ...
    # EXPLICITLY LOAD PPO CHECKPOINT
    ckpt = "checkpoints/ppo_model_best.msgpack" 
    try:
        params = load_checkpoint(ckpt)
        print(f"✅ Loaded PPO weights from {ckpt}")
    ...
    pygame.display.set_caption("JAX Scenarios — Evaluation")
    ...
    while True:
        ...
        env_action = scale_action_to_env(jnp.squeeze(mean, axis=0), float(stacked_state.env_state.max_v))
